...
public class SelectQuery extends SQLQuery<List<Row>> {
    ...
    public SelectQuery(Statement statement, DataSource dataSource) {
        this(statement, new QueryRunner(dataSource), new RowHandler());
    }

    public SelectQuery(Statement statement, QueryRunner run, AbstractListHandler<Row> handler) {
        super(statement, run);
    ...
    protected List<Row> run() throws SQLException {
        return run.query(this.statement.getQueryString(), handler, this.getParamValues());
    }
}
